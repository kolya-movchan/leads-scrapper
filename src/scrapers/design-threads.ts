...
const KEYWORDS = process.env.DESIGN_THREADS_KEYWORDS
  ? process.env.DESIGN_THREADS_KEYWORDS.split(",")
      .map((k) => k.trim())
      .filter(Boolean)
  : DEFAULT_KEYWORDS;
...
const MAX_POSTS = Math.max(
  10,
  Number(process.env.DESIGN_THREADS_MAX_POSTS) || 15,
);
...
export async function scrapeDesignThreads(): Promise<RawPost[]> {
  ...
  const startRes = await axios.post(
    ...
    input,
    {
      ...
      timeout: (WAIT_FOR_FINISH_SECS + 30) * 1000,
    },
  );
  ...
  while (!terminal.includes(run.status) && polls < maxPolls) {
    ...
    const pollRes = await axios.get(
      `${APIFY_BASE}/actor-runs/${run.id}?token=${token}`,
    );
  ...
  if (run.status === "RUNNING") {
    const abortRes = await axios.post(
      `${APIFY_BASE}/actor-runs/${run.id}/abort?token=${token}`,
      {},
    );
    run = abortRes.data.data;
    console.warn(
      `[design-threads] run ${run.id} exceeded ${MAX_POLL_MINUTES}min, aborted; using partial results`,
    );
  }
